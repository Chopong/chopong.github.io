...
chopong_showSubToc = function () {
  ...
  if (all_content) {
    ...
    res = cur_expand ? all_content[cur_expand].innerHTML : "Nothing";
  };
}; // 官方提供的方法不兼容，自定义函数
...
*/
chopong_pagination = function(){
  page_num = document.querySelectorAll('.pagination>li').length - 2;
  toggle_some_page = function(ipage,t){
    document.getElementById('page-'+ipage).setAttribute('class',"list-group flex-column collapse "+t);
  };
  let_some_page_index_light = function(ipage,t){
    document.getElementById('button-'+ipage).parentNode.setAttribute('class',"page-item " + t);
  };
  collapse_all_page = function(){
    for( i = 1 ; i <= page_num ; i++){
      toggle_some_page(i,"");
      let_some_page_index_light(i,"");
    }
  };
  refresh_pre_next_btn = function(){
    if(!document.querySelector(".list-group.flex-column.collapse.show")){
      toggle_some_page(1,'show');
      let_some_page_index_light(1,"active");
    };
    $('#button-pre').on('click', function (e) {
      e.defaultPrevented;
      curr = document.querySelector(".list-group.flex-column.collapse.show");
      this_id = parseInt(curr.id.substring(5));
      pre_id = this_id - 1;
      if (this_id != 1) {
        collapse_all_page();
        toggle_some_page(pre_id, "show");
        let_some_page_index_light(pre_id,"active");
      }
    });
    $('#button-next').on('click', function (e) {
      ...
      if (this_id != page_num) {
        collapse_all_page();
        toggle_some_page(next_id, "show");
        let_some_page_index_light(next_id,"active");
      }
    });
  };
  click_this_btn = function(i){
    $('#button-'+i).on('click', function (e){
      e.defaultPrevented
      collapse_all_page();
      toggle_some_page(i,"show");
      let_some_page_index_light(i,"active");
    });
  };
  refresh_pre_next_btn();
